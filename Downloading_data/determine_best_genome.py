...
def process_call_from_script(argv):
	command_line_settings, args = process_command_line(["-s","Downloading_data/determine_best_genome.sh"])
...
def main(argv=None):
	...
	#run script
	call([settings.installation + settings.script, settings.EDIRECT, settings.QUAST, settings.mail, settings.organism, settings.assembly_file, settings.workdir, no_deletion])
        #print([settings.installation + settings.script, settings.EDIRECT, settings.QUAST, settings.mail, settings.organism, settings.assembly_file, settings.workdir, no_deletion])
	return 1
